...
Сервер не подключается напрямую к боевой базе данных ЦПР. Он читает только публичный API:

`https://apiiola.yasg.ru/api/v1`

Это важно для безопасности: наружу не раскрываются IP базы данных, исходные схемы, служебные таблицы и непубличные поля.

## Публичный адрес MCP

После деплоя сервер доступен по адресу:
